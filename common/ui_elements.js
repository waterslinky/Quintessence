...
class Item_display extends button{
    constructor(info){
        ...
        this.hover_func = function(){
            ...
            if(item.item.name!="blank" && engin.cursor.image==undefined){
                ...
                if(item.item.display_name && this.type!="bar_slot"){
                    
                    if(typeof item.item.display_name=="object"){
                        item_name_display.text = item.item.display_name
                    }
                    else{
                        item_name_display.text = [item.item.display_name]
                    }
                    ...
                    if(item.item.has_knowledge){
                        ...
                        player.knowledge.forEach(knowledge => {
                            if(knowledge.name==item.item.name){
            ...
            if((item.item.name=="blank" || !item.item.display_name) || this.type=="bar_slot" || player.inventory_hand.item.name!="blank" ){
        ...
        this.on_clicked = function(){
            ...
            if(this.type=="slot"){
                ...
                if(player.inventory_hand.item.name==this.item.item.name){

                    player.inventory_hand.set_count(Entity_class.give_item(player.inventory_hand,[item]))
                    ...
                    player.inventory_hand.item=copy(item.item)
                    // this.hand_size=.6
                
                    item.item=old_player_inventory_hand.item
    
                    player.inventory_hand.count=item.count
                    item.count=old_player_inventory_hand.count                
                }
            ...
            if(this.type=="infinite_output"){
                if(player.inventory_hand.item.name!="blank"){
                    player.inventory_hand.item=get_just_block("blank")
                    ...
                    player.inventory_hand.set_count(get_property(player.inventory_hand,"stack_size"))
        ...
        this.on_right_clicked = function(){
            ...
            if(this.type=="slot"){
                ...
                if(item.item.name!="blank"  && item.item.name!=player.inventory_hand.item.name && player.inventory_hand.item.name=="blank"){
                    // console.log(1)
                    player.inventory_hand.item=copy(item.item)
                    player.inventory_hand.set_count(Math.ceil(item.count/2))
                    item.set_count(Math.floor(item.count/2))
    
                }
                else if(player.inventory_hand.item.name!="blank" && (item.item.name==player.inventory_hand.item.name || item.item.name=="blank") ){
                    // console.log(2)
                    if(item.item.name=="blank" ){
                        item.set_count(0)
                    }
                    item.item=copy(player.inventory_hand.item)
                    // player.inventory_hand.set_count(.count-1)
                    
                    item.give_count(1,player.inventory_hand)
                }
            ...
            if(this.type=="infinite_output"){
                ...
                if(item.item.name!="blank" && player.inventory_hand.item.name=="blank"){
        ...
        this.calculate_size = function(){
            ...
            this.display_size_x=(item.item.hand_size*this.size_x)*this.item_size_mult
            this.display_size_y=(item.item.hand_size*this.size_y)*this.item_size_mult
        ...
        this.draw = function(){
            ...
            if(typeof this.image.length!="undefined"){
                for(let i=0;i<this.image.length;i++){
                    ...
                    if(this.item.item.image_offsets && this.item.item.image_offsets[i]){
                        x += (this.item.item.image_offsets[i][0]*0.0625)*size_x
                        y += (this.item.item.image_offsets[i][1]*0.0625)*size_y
                    }
...
// }
        ...
        if(typeof this.inventory_parent.length=="undefined"){
            this.item = this.inventory_parent
        }
        else{
            this.item = this.inventory_parent[this.index]
        }
