...
class Entity_class{
    ...
    static give_item(item,count=1,inventory = player.inventory){
        let stack_found=false
        for(let i=0;i<inventory.length;i++){
            if(!stack_found){
             
                if(inventory[i].item.name==get_item(item).name){
                    let pass=true



                    if(inventory[i].count){
                        if(inventory[i].count+count>stack_size){
                            
                            count-=(stack_size-inventory[i].count)
                            inventory[i].count=stack_size
                            pass=false
                        }
                        else{
        ...
        if(!stack_found){
            for(let i=0;i<inventory.length && stack_found==false;i++){
                if(inventory[i].item.name=="blank"){
                    inventory[i].item=get_item(item)
                    inventory[i].count=count
...
class player_class{
    constructor(){
        ...
        this.give_item = Entity_class.give_item
    ...
    reset_inventory(){
        for(let i=0;i<4*9;i++){
            ...
            try{
                ...
                this.inventory[i]=new inventory_slot(get_item(old_item.item  ))
                this.inventory[i].count=old_item.count
                
            }
            catch{
                this.inventory.push(new inventory_slot(get_item("blank")))
            }
        ...
        this.inventory_hand=new inventory_slot(get_item("blank"))
        ...
        if(this.game_mode!="AscendedGost"){        
            for(let x=Math.floor(this.x/block_size);x<Math.ceil((this.x+this.size_x)/block_size);x++){
                for(let y=Math.floor(this.y/block_size);y<Math.ceil((this.y+this.size_y)/block_size);y++){
                    ...
                    if(block_list[x][y].collision_box){



                        if(this.x+this.size_x>(x)*block_size && this.old_x+this.size_x<=(x)*block_size){
                            // this.x=((x)*block_size)-this.size_x
                            if(!block_list[x-1][y-1].collision_box && !block_list[x][y-1].collision_box && this.grouded){
    ...
    for(let i=0;i<trait_list.length;i++){
        ...
        if(trait=="inventory"){
            ...
            player["inventory"].forEach(item => {
                ...
                let item_name = typeof item.item.name!="undefined" ? item.item.name : "blank"
