...
//             screen.globalCompositeOperation = 'destination-in'
            ...
            let old_count = inventory_hand.item.count
            inventory_hand.item.count=0

            inventory_hand.draw()
            inventory_hand.item.count=old_count
        ...
        this.add_item = function(item,x,y){
            ...
            recipes.forEach(recipe => {
                ...
                if(correct_table){
                    ...
                    if((ingredient.item && ingredient.item.name==item.item.name) || (item.item.tool_part && item.item.tool_part.part==ingredient.part_type)){
        ...
        let drop_item_on_table=function(table_ui=this,count=1){
            ...
            if(player.inventory_hand.item.name!="blank" && can_place){
                ...
                if(in_on_top_of_item==false){
                    let item = copy(player.inventory_hand)
                    ...
                    if(hits==4){
                        ...
                        this.closest_item.item.parent_reciple.recipe.result.forEach(result_item => {
                            ...
                            if(result_item.tool){
                                ...
                                this.add_item(
                                    {
                                        "item":get_item(result_item.tool,{"parts":parts}),
                                        "count":1
                                    },
                                ...
                                this.add_item({
                                    "item":get_item(result_item.item.name),
                                    "count":1
                                },
