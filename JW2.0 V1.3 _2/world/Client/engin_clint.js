...
function draw_block(block,x,y,size_x,size_y){
    if(block && block.image!=undefined){
        let image_px_width

        // console.log(block)
        try{
            image_px_width = typeof block.image.length!="undefined" ? block.image[0].width : block.image.width
        }
        catch(err){
            console.log(block)
        ...
        // console.log(block)
        let image = typeof block.image.length!="undefined" ? block.image[0] : block.image
        ...
        if(image!=undefined){
            ...
            if(dir!=undefined){
                ...
                draw_image(block.image,px_x,px_y,px_width,px_height,width*-.5,height*-.5,width,height);
                
                screen.restore()
            }
            else{
                try{
                    draw_image(block.image,px_x,px_y,px_width,px_height,x,y,width,height);
                }
                catch(err){
...
function block_to_image(block){
    ...
    if(typeof block.image.length == "number"){

        image_object.image = block.image
    }
    else{
        image_object.image = [block.image]
        ...
        if(-1<x){
            for(let y=start_y;y<hieght;y++){
                if(-1<y){
                    ...
                    if(all_blocks[x][y].side_image || all_blocks[x][y].left_side_image || all_blocks[x][y].top_side_image || all_blocks[x][y].bottom_side_image || all_blocks[x][y].right_side_image){
                        ...
                        if(up_block!=undefined && up_block.name=="air"){
                                // if((x-start_x)>=0 && (y-start_y)-1>=0 && x-start_x<=all_blocks.length-1 && (y-start_y)-1<=all_blocks[0].length-1){
                                    ...
                                    if(all_blocks[x][y].side_image){

                                        side_block={
                                            "image":all_blocks[x][y].side_image,
                                            "dir":0
                                        }
                                        
    
                                    }
                                    else if(all_blocks[x][y].top_side_image){
                                        
        
                                        side_block={
                                            "image":all_blocks[x][y].top_side_image,
                                            "dir":0
                                        }
                        ...
                        if(right_block!=undefined && right_block.name=="air"){
                            ...
                            else if(all_blocks[x][y].right_side_image){
                                        
                                side_block={
                                    "image":all_blocks[x][y].right_side_image,
                                    "dir":0
                                }
                        ...
                        if(down_block!=undefined && down_block.name=="air"){
                                ...
                                else if(all_blocks[x][y].bottom_side_image){
                                    side_block={
                                        "image":all_blocks[x][y].bottom_side_image,
                                        "dir":0
                                    }
                                }
                        ...
                        if(left_block!=undefined && left_block.name=="air"){
                                if(x-start_x-1>=0 && (y-start_y)>=0 && x-start_x-1<=all_blocks.length-1 && (y-start_y)<=all_blocks[0].length-1){
                                    ...
                                    else if(all_blocks[x][y].left_side_image){
                                        side_block={
                                            "image":all_blocks[x][y].left_side_image,
                                            "dir":0
                                        }
                                    }
            ...
            for(let y_index=0;y_index<display_layer[x_index].length;y_index++){
                ...
                if(block!=undefined){
                    ...
                    block_layers.forEach(block => {
                        draw_block(block,Math.round(x+(x_index*block_size)),Math.round(y+(y_index*block_size)),(Math.round(x+((x_index+1)*block_size))-Math.round(x+(x_index*block_size))),Math.round((y+((y_index+1)*block_size))-Math.round(y+(y_index*block_size))))
                    });
...
// eval(("."+element))  old_player_from_ping[element]
        ...
        if(mining){
            ...
            if(player.game_mode=="Servival"){
                ...
                if(player.block_brocking){
                    let break_block_hit_box_index=block_list[player.block_brocking[0]][player.block_brocking[1]].hit_box_index
                    let world_mouse_hit_box_index=block_list[world_mouse_x][world_mouse_y].hit_box_index
                ...
                if(((player.block_brocking[0]!=world_mouse_x || player.block_brocking[1]!=world_mouse_y) && !same_block) || player.selected_slot_index.index!=player.break_with_slot.index){
                    ...
                    if(block_list[world_mouse_x][world_mouse_y].destroy_time!=undefined){

                        let mining_mult = 1

                        let held_item_tool_type = player.inventory[player.selected_slot_index.index].item.tool_type
                        let block_brocking = block_list[world_mouse_x][world_mouse_y]

                        if(block_brocking.broken_with && held_item_tool_type){

                            block_brocking.broken_with.forEach(element => {
                        ...
                        player.end_break=engin.time_in_loop+((block_list[world_mouse_x][world_mouse_y].destroy_time*1000)/mining_mult)
                ...
                if(player.block_brocking && engin.time_in_loop>=player.end_break){
                    ...
                    let tool_types = player.inventory[player.selected_slot_index.index].item.tool_type

                    let loot_table = block_list[player.block_brocking[0]][player.block_brocking[1]].loot_table

                    let replace_with = "air"
                    
                    if(destroy_block.broken_with && tool_types){

                        tool_types.forEach(tool => {

                            destroy_block.broken_with.forEach(element => {
        ...
        if(is_placing){
            ...
            if(block_list[world_mouse_x][world_mouse_y].on_right_clicked && !old_is_placing){
                block_list[world_mouse_x][world_mouse_y].on_right_clicked()
                ...
                if(player.inventory[player.selected_slot_index.index].item.name!="blank" && player.inventory[player.selected_slot_index.index].item.type=="block" && blank_area(world_mouse_x,world_mouse_y,(typeof player.inventory[player.selected_slot_index.index].item.hit_box!="undefined" ? player.inventory[player.selected_slot_index.index].item.hit_box[0] : 1),(typeof player.inventory[player.selected_slot_index.index].item.hit_box!="undefined" ? player.inventory[player.selected_slot_index.index].item.hit_box[1] : 1))){

                    if(player.inventory[player.selected_slot_index.index].count>=1){
                        if(typeof player.inventory[player.selected_slot_index.index].item.on_used=="undefined"){
                            change_block(world_mouse_x,world_mouse_y,player.inventory[player.selected_slot_index.index].item.name)
                        
                            if(player.game_mode=="Servival"){

                                player.inventory[player.selected_slot_index.index].set_count(player.inventory[player.selected_slot_index.index].count-1)
                ...
                if(typeof player.inventory[player.selected_slot_index.index].item.on_used!="undefined"){
                            if(typeof player.inventory[player.selected_slot_index.index].item.duration_ends=="undefined"){
                                player.inventory[player.selected_slot_index.index].item.duration_ends=engin.time_in_loop+player.inventory[player.selected_slot_index.index].item.use_after_duration
                            }
                            else{
                                if(engin.time_in_loop>=player.inventory[player.selected_slot_index.index].item.duration_ends){
                            
                                    if(player.inventory[player.selected_slot_index.index].item.on_used){
                                        let returned = player.inventory[player.selected_slot_index.index].item.on_used()

                                        if(returned || typeof returned=="undefined"){
                                            player.inventory[player.selected_slot_index.index].set_count(player.inventory[player.selected_slot_index.index].count - (typeof player.inventory[player.selected_slot_index.index].item.decrease_after_use!="undefined" ? player.inventory[player.selected_slot_index.index].item.decrease_after_use : 0))
                                        }
                                    ...
                                    player.inventory[player.selected_slot_index.index].item.just_used=true
                                    player.inventory[player.selected_slot_index.index].item.duration_ends=undefined
            ...
            if(old_is_placing && typeof player.inventory[player.selected_slot_index.index].item.on_used!="undefined" && engin.time_in_loop<=(player.inventory[player.selected_slot_index.index].item.duration_ends-player.inventory[player.selected_slot_index.index].item.use_after_duration)+200 && (typeof player.inventory[player.selected_slot_index.index].item.just_used!="undefined" && !player.inventory[player.selected_slot_index.index].item.just_used)){
                // player.inventory[player.selected_slot_index.index].item.on_used()

                console.log("V")
                if(player.inventory[player.selected_slot_index.index].item.name!="blank" && blank_area(world_mouse_x,world_mouse_y,(typeof player.inventory[player.selected_slot_index.index].item.hit_box!="undefined" ? player.inventory[player.selected_slot_index.index].item.hit_box[0] : 1),(typeof player.inventory[player.selected_slot_index.index].item.hit_box!="undefined" ? player.inventory[player.selected_slot_index.index].item.hit_box[1] : 1))){
                    if(player.inventory[player.selected_slot_index.index].count>=1){
            ...
            player.inventory[player.selected_slot_index.index].item.just_used=false
